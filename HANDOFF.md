...
| `data/` | 运行时数据（不提交，但随目录移动）：`trades/`、`orders/`、`soak/`、`shadow/`、`backup-*` |
...
每笔固定 **10 股**（`min_shares=max_shares=10`，无 CLI 可调）；单笔成本 ≈ $4.3–4.5。
...
- **实盘资金门槛**：每笔 10 股 ≈ $4.5 → 1 仓需 ~$5、2 仓需 ~$10，建议 $12–15。若只给 4.8u，
  需把 `min_shares/max_shares` 降到 ~4 股（需加 CLI，尚未做）。
- **胜率 vs 盈亏比**：不可兼得（§28）。当前取舍：盈亏比优先（PF~2.9，WR~63%）。
