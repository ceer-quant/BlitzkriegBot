...
- **交付物**
  ...
  2. 事件驱动回测器 ✅ **已完成（P-1.2，`MIGRATION_LOG §35`）**：`Backtester` trait + 全链路重放
     （行情→信号→风控→账本→OME→出场，`EventBacktester` 在虚拟时钟上驱动真实 `Core`），
     支持滑点/延迟/成交概率模型（`FillModel`，默认恒等 = 行为不变）。
  3. 数据抽象 ✅ **已完成（P-1.3，`MIGRATION_LOG §35`）**：`DataSource`/`DataSink` trait +
     `--event-archive` JSONL 归档（L2/tick/spot/round，Decimal 字符串精确编码）+ `--backtest` 回放；
     回测与 live 共用 `Core::engine_on_data` 这同一 feed 入口。
  4. 影子进化保真度修复 ✅ **已完成（`MIGRATION_LOG §33`）**：定向变异、同时刻盘口、统一 `ExitConfig`。
- **验收**：同一策略在 live 与回测上对同一历史区间给出一致 PnL（±容差）；两种策略可同时运行并各自记账
  （多策略 ✅ 已满足：`strategy.list` 可见两策略、各自 `strategies[]` 账本互不混淆；
  **live/回测一致性 ✅ 已达成**：`scripts/backtest-check.mjs` **21/21**——同一次采集中 live 与 `--backtest`
  的净 PnL **逐位相等**（5.12208717）、订单计数/feed 计数/策略分账完全相同；真实 feed 归档
  （1 025 963 事件 / 13 分钟）重放 **19/19**：归档逐类行数 == 回放 feed 计数、`blocked.momentum` 88=88、
  `blocked.timing` 810 vs 805（0.6%，宿主定时器相位）、`confirmed` 诊断逐值一致，同参数两次回放报告逐字节相同。
  详见 `MIGRATION_LOG §35`（含真机回放暴露并修复的两个回测保真度缺陷）。
...
> **当前执行计划以 `docs/ROADMAP_V0_1.md` 为准**（GitHub Milestone `v0.1`）。本节是机构级
> 长期收敛建议（P-1..P-6）；0.1 阶段只做该文件列出的 E1–E6，本表第 3 项之后的 P-2..P-6
> 事项**登记不执行**。

| 序 | 事项 | 理由 |
|---|---|---|
| 1 | **多策略执行打通（P-1.1）** ✅ 已完成（`MIGRATION_LOG §34`） | 抽象已就绪却无法影响交易——投入最小、解锁最多 |
| 2 | **事件驱动回测 + 数据抽象（P-1.2/1.3）** ✅ 已完成（`MIGRATION_LOG §35`） | 没有它，任何策略/参数改动都不可验证 |
| 3 | **多策略底座 + 对冲策略 + 策略重构（`ROADMAP_V0_1` E2/E3/E4）** ← 下一项 | 规模化与「不靠单一策略」的前提；P-2/P-3 的完整形态留到 0.3 |
