...
## §53 仓库开源前的脱敏：一次「校验方式本身失效」的事故

事故等级：**已推送到公开仓库的静默损坏**，涉及 417 个文件、46 个分支。
没有数据丢失（有改写前备份），但在一段时间内公开仓库的当前内容是坏的。

### 过程

为开源做脱敏，用 `git filter-repo --replace-text` 把所有分支上的本机
路径与用户名替换成占位符。改写后按惯例校验：**账本指纹未变、文件数
未变、提交数未变**——全绿，于是推送。

之后在排查另一件事（评估 legacy 文件能否删除）时偶然发现：
`src/bin/worker.ts` 第 1 行是 `***REMOVED***!/usr/bin/env node`。

### 根因

`--replace-text` 从**表达式文件**读规则，而**以 `#` 开头的行会被当作
字面替换规则，不是注释**。规则文件里本来写了一整段说明注释，于是
「`#` → `***REMOVED***`」这条能力被无意中注册，作用范围是全仓每一个
`#`。
...
| 类别 | 例子 | 是否功能性 |
|---|---|---|
| shebang 被毁 | `#!/usr/bin/env node` | **是**——脚本直接执行会挂 |
| 语言注释被毁 | `# 已知问题`、YAML/shell 注释 | 否，但可读性归零 |
| **代码里的 `#`** | `config.replace(/^#?PasswordAuthentication\s+.*/m, ...)` 变成 `/^***REMOVED***?PasswordAuthentication.../` | **是——静默的安全回归** |

第三类最危险：正则仍然合法、仍然执行、只是永远不匹配，所以 SSH 加固
**看上去还在跑**。

### 为什么校验放行了

我用的三条不变量（账本指纹、文件数、提交数）**都无法观测到这次损坏**：
`***REMOVED***` 不改变文件个数，账本文件本身没被碰到，提交数当然也没变。

**这是这次事故真正的教训**：不是「忘了检查 `#`」，而是**我选的不变量
与这次改动的失败模式不相关**。`--replace-text` 的风险恰恰是「内容被
改坏」，而我的校验里没有一条是关于内容的。

正确的校验是**内容级的**：把结果逐文件归一化回原字符串，再与改写前
比对，归一化后应当**完全一致**。本次改用这个方法后，18 个差异文件里
14 个归一化后一致（纯脱敏），4 个不一致且**都是我有意做的、逐个复核
过的编辑**。

### 处置

1. **改掉工具而不是改掉症状**：不再用带表达式文件的 `--replace-text`，
   改为直接对目标文件做字面替换的脚本。脚本没有表达式文件，因此
   「注释行变成规则」这个失败模式**在结构上不存在**——这是结构性修复，
   不是「这次记得别写注释」。
2. 从改写前备份恢复**全部 46 个分支**与工作区。恢复前先证明
   「受损版本 = 干净版本 经 `#` 被吞 + 本机路径脱敏两种变换」，逐分支
   归一化比对，42/46 完全一致，另 4 个的差异经逐条复核均属预期。
3. 重新应用脱敏（12 文件 21 处替换）与 `.zcode`/`HANDOFF.md` 移出版本库。
4. 重跑全部门禁后才允许再次触碰历史：typecheck 0 错（损伤态曾是 12 错）、
   172 测试全过、build 成功、`plutil -lint` OK、三个 core 门禁 PASS、
   secret-scan 干净、全仓 `***REMOVED***` 残留 0、60 个脚本 shebang 完整、
   账本指纹 `91b612c91cb084ccc01cad7b58671957` 未变。

### 可复用的教训

- **备份先于一切破坏性操作，并且要验证备份可恢复**。这次能全身而退，
  唯一原因是改写前做了 bundle + 镜像 clone 并且**验证过能恢复**。
- **校验的不变量必须与改动的失败模式相关**。文件数对「内容被改坏」
  是无效证据。写完校验先问一句：**这次改动可能以什么方式坏掉？那个
  坏法会让我的不变量发生变化吗？**
- **静默失效比崩溃危险**。正则被改坏后仍然合法、仍然执行，只有结果
  不对。凡是「装上了」的断言，都要同时断言「它真的产生了预期效果」。
- 工具的参数语义要按字面理解并验证：`--replace-text` 的「注释」不是
  注释。**用不了解语义的参数做全仓批量替换，等于全仓盲写。**
