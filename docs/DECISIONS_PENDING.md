...
> **已决议（2026-09-15）**：
> - **D-13.1 / D-13.2 → 维持（选项 A = 选项 C 的「盐保留」半边）**。用户口径：*由新版
>   BlitzkriegBot 创建的交易不应遗留任何 cloddsbot 痕迹；过去测试/开发期间遗留的历史交易
>   不再追溯、不再修改*。即新链上锚点若换记法，只能以「仍校验旧前缀的新版本」形式引入
>   （双读），不得就地改老前缀；加密盐原样保留，两侧均已加 in-code 注释登记
>   （`src/security/index.ts` salts 派生处、`src/ledger/anchor.ts` memo 构造处）。
> - **D-13.3 → 已执行**：本地 `origin` remote 已移除（2026-09-15），`.git/config` 中
>   clodds 命中数为 0；上游比对能力按用户指示放弃。
...
### 附：F1 校准（2026-09-15，#29 E3-a §4 步骤 1）
- 同一留出段（`events.20260915T111257Z.jsonl`，1,948,329 事件）上，`--trend-confirm-sec` 60/10/20 三腿
  **逐位相同**（10 订单/6 成交/3 平仓，1W/2L，net −$1.914，PF 0.115；信号 319、momentum 拦截 1005、
  timing 44）；600 s 外推对照腿确实变化（1 平仓），证明旋钮接通。
- 读法：top 流观测密度亚毫秒级，10 s 窗已满足确认条件（ratio × 跨度），窗口在 10–60 s 区间决策中性；
...
## [待决策] D-23 GitHub 账号计费问题导致 CI 全面停摆（阻塞合并流程）

- **背景**：`7a66689`（PR #103，`feat/e12-readonly-egress`）推送后，
  `CI` 与 `Security` 两个 workflow 的**全部 job** 在约 2 秒内判 failure，
  但 `steps: []`、`runner_name: ""`、`runner_id: 0`——**没有任何 runner 被分配**。
  重跑三次（`5670e4e` 两次、`7a66689` 一次）结果一致。

  **确证根因**（不是推测）：`steps: []` 时 job 日志是 `BlobNotFound`（未写出），
  ...
  **时间线**：`709afb9`（06:40）6 job 全绿 → `c4890a4`（06:52）5 个真实 job 全绿、
  仅 `notify` 无 runner → `5670e4e`（07:25）起**全部 job** 无 runner。
  即 06:40–07:25 之间账号可用额度耗尽。

  **为什么这需要用户拍板**：既定工作流是「分支 → 本地门禁 → PR → **CI 全绿** →
  squash 合并」。现在 CI **无法执行**，于是 PR #103 卡在一个**流程无法满足**的状态：
  它的 CI 结论是**空的**（既非绿亦非红），而「本地全绿」按既定流程**不足以**替代。
  AI 不自行放宽这条口径，故记录待决。

- **选项 A**：**等**。用户处理账号 Billing（补付欠款/提高 spending limit），
  然后重跑 PR #103 的真实 CI，按既定流程合并。
  最保守，不产生任何「依本地证据合并」的先例。
- **选项 B**：**本次例外**，依已完成的本地证据合并 PR #103
  （本地逐条重放 `rust-check` / `node-check` / `panel-check` 全部通过，
  232 Rust + 172 node 测试、7 个门禁脚本全 PASS，且 `--readonly` 有对抗式门禁 +
  对照实验证明非恒真），**并明确记录这是一次性例外**，同时立即处理账号额度。
- **选项 C**：**临时降低 CI 依赖**——把门禁改为本地强制 + CI 只在有 runner 时执行，
  并把「CI 未执行」与「CI 通过」在 PR 模板/状态里**显式区分**，避免空 CI 被读成绿。
- **AI 倾向**：**A，但设一个明确的等待上限**。理由：
  1. #94 的硬约束含「禁止未验证就提交主分支」；**空 CI ≠ 已验证**，
     而「本地跑过」与「CI 门禁存在」是两件事——本次本地重放是**被迫的降级**，
     把它当成等价物会在下一次真正需要 CI 时掏空这条防线。
  2. PR #103 是 `--readonly` 这种**安全相关**的改动（结构性禁下单），
     恰恰是最不该在没有独立验证通道时合并的类型。
  3. 但若额度短期无法恢复，长期悬挂一个有完整本地证据的 PR 也不是好事，
     故建议：**等待上限到期后回到用户面前重新决策**，而不是自动转 B。
- **需要用户确认的点**：
  1. 选 A（等额度恢复后走完整 CI）还是 B（本次例外合并）？
  2. 账号 Billing 你是否会处理？预计时间？（这决定 A 是否可行）
  3. 若同意 C 的方向：是否要把「CI 未执行」做成 PR 上的**显式状态**，
     以免将来有人把空 CI 误读成绿？（KI-26 已记录该误读风险，
     且本轮已在 `c4890a4` 上**实际误读过一次**）
  4. **注意**：给 `notify` 加 `continue-on-error: true` **不在**建议之列——
     那只会让真实的通知故障静音，掩盖问题而非解决问题。

---

### D-24：CloddsBot 遗留代码的处置范围

**背景**：仓库前身是 CloddsBot（作者 `alsk1992` / `ALSK`）。用户已明确
「与他有关的代码可以被全部删除了」，并要求「只留你自己的提交」。

**实测事实（决定了这件事不能简单做）**：

当前 `main` 的 1105 个文件，按「最后一次改动者」分类：

| 改动者 | 文件数 | 含义 |
|---|---|---|
| `alsk1992` | 522 | CloddsBot 时期最后改动 |
| `fancer` | 506 | 我们改过 |
| `ceer_quant` | 71 | 我们改过 |
| `ALSK` | 6 | CloddsBot 时期最后改动 |

528 个「最后改动者是 CloddsBot 作者」的文件里，**235 个被当前代码 import**。
依赖最重的几个：

- `src/utils/logger.ts` —— **355 个文件**依赖它
- `src/utils/id.ts` —— 25 个
- `src/pairing/index.ts` —— 21 个
- `src/trading/safety.ts`、`src/trading/secrets.ts` —— 被 `src/gateway/index.ts`
  等**我们自己的**文件引用

沙箱实测（`/tmp/tscheck` worktree + 完整 `node_modules`）：删掉 484 个
legacy `src/` 文件后，`tsc --noEmit` 只报 12 个错，且**全部来自 shebang
损伤**（`***REMOVED***!` 不是合法 TS），**没有一个来自缺失的 import**。
说明这些文件是被入口文件调用着的**实现层**，不是死代码。

**另有一条历史改写的事实**（与 D-24 相关但独立）：删提交会让
「由被删提交引入、之后再未改动过」的文件**一并消失**。实测删掉
CloddsBot 时期提交后，`main` 文件数从 1105 掉到 577，**丢失 `LICENSE`、
`src/`、`tests/`、`tsconfig.json`、`assets/`、`.github/dependabot.yml`**。
即「只删提交」做不到「只删来源」，会连带删掉当前项目仍在用的文件。

**选项**：

- **选项 A**：**不做批量删除**。CloddsBot 的历史来源在文档里说明清楚即可
  （已由 §53 与开源说明覆盖），代码照常演进。零风险，但仓库里始终存在
  作者归属混杂。
- **选项 B**：**按功能模块渐进下线**，一个模块一个 PR，每个 PR 必须
  同时摘掉对它的所有 import 并保持门禁全绿。先做**零引用**的（沙箱已
  证明不存在整批零引用，所以这需要先跑出真实的零引用清单——注意
  `src/trading/commands.ts` 这类**已不存在的文件仍有死引用**，说明目录
  里还有陈旧引用需要先清理）。
- **选项 C**：**重写为「快照导入提交」**——把 CloddsBot 时期压成**一个**
  合成的根提交（保留其最终树，从而 `LICENSE`、`src/` 等文件不丢），
  上面再挂我们自己的提交。既去掉 CloddsBot 的作者归属与历史，又不丢文件。
  代价：历史起点变成一个无作者信息的导入点，需要接受并在文档记录
  （用户已就「接受，并在文档里记录」表态）。

**AI 倾向**：**C + B 的组合，且 C 先把「丢文件」这个洞堵上**。理由：

1. 用户的真实意图是「CloddsBot 的代码不该出现在这个仓库里」——这是
   **内容**问题，而删提交解决的是**历史**问题，且会顺带删内容。
   C 能同时满足「无 CloddsBot 作者/历史」与「不丢文件」。
2. 但 C **只解决作者归属，不解决代码来源**。`src/solana/`、`src/percolator/`、
   22 个 channel 适配器这些显然不服务于「Polymarket/Kalshi HFT 内核」的
   模块，是否要留是**产品判断**，不是清理判断。
3. 因此建议：**先做 C 把历史问题一次解决**，再用 B 逐个模块决定去留；
   不要试图用一次批量删除同时解决两件事。

**需要用户确认的点**：

1. 选 A / B / C？（若选 C，是否接受历史起点变为无作者信息的导入提交）
2. 若走 B：`src/solana/`（约 25 文件）、`src/percolator/`（10 文件）、
   `src/bittensor/`（7 文件）、22 个 channel 适配器——哪些模块确认不要？
   这些**都不是死代码**，删它们要连带改调用方，属功能性变更。
