...
- **选项 B**：A + 把「止损超调」变为可观测——平仓时把触发线/当刻 bid/mid 写进 trade 记录
  ...
  > **已决议（2026-09-14 晚）：常开。** 轮转 + 磁盘护栏已实现并接入生产（`MIGRATION_LOG §36`）：
  > 默认 `data/archive/events.jsonl`、每段 256 MB、无会话上限、可用空间 <5 GB 即停录；
  > `HFT_EVENT_ARCHIVE=off` 可关。**逐笔盘口复盘能力已具备**（选项 B 的观测部分已落地），
  > 因此剩余待决仅：是否再加「触发线 + 当刻 bid/mid 写进 trade 记录」这类**成交记录内的**字段
  > （归档能事后复现，但查一笔单仍需跑一次 `--backtest`），以及何时据分布调参（选项 C）。
...
- 因此上面的**选项 B 已具备工具**：常开 `--event-archive` → 事后 `--backtest` 逐笔复现某笔止损的盘口路径，
  ...
  （1）是否把面板负号修掉（1 行，`ui/hft.html:444`）；（2）是否在生产 dry 内核常开归档（速率 ≈16 GB/天，需轮转）。
