...
> | [`blitzkrieg/MIGRATION_LOG.md`](./blitzkrieg/MIGRATION_LOG.md) | **已修复**缺陷的变更史（§1–§47） | 未修复项 |
>
...
**最后核对：2026-09-17，针对 `main` @ `4c4b351`。**

> ⚠️ **本文件写作时，工作树中有另一条工作线的未提交改动**（`OrderRole` 成交角色/费用记账，
> 涉及 `core/` 12 个文件 + `extensions/` + `src/core/`）。**本文件的全部结论均以已提交的
> `HEAD` 为准**，不受工作树改动影响。接手时先 `git status` 看清工作树。
...
| 编号 | 一句话 | 严重度 | 状态 |
| --- | --- | --- | --- |
| KI-1 | dry 挂单永不成交，入场全 taker（测量基准错） | 🔴 | 已裁决待实现 |
