...
Blitzkrieg uses a SKILL.md-based skill system for extending the AI agent with new capabilities. Skills are markdown files with YAML frontmatter that define commands, documentation, and requirements.
...
| 2 | `.blitzkrieg/skills/` | User-managed skills |
...
When a user types `/himalaya list`, Blitzkrieg routes directly to the Bash tool with `himalaya list` instead of sending it through the AI.

---

## OpenClaw Compatibility

Blitzkrieg accepts OpenClaw-format SKILL.md files with zero changes. All OpenClaw features are supported:

### OpenClaw Metadata Block

OpenClaw stores extended metadata in a JSON/JSON5 `metadata` field under a manifest key (`openclaw`, `blitzkrieg`, or `clawdbot`):
...
| OpenClaw Field | Blitzkrieg Equivalent | Behavior |
|---------------|-------------------|----------|
...
If both Blitzkrieg-native `gates` and OpenClaw `requires` are present, they're merged (deduplicated).

### Compatibility Matrix

| Feature | Blitzkrieg Native | OpenClaw Format | Works? |
|---------|--------------|-----------------|--------|
...
cp -r ~/openclaw-skills/himalaya .blitzkrieg/skills/himalaya
```

The folder should contain at minimum a `SKILL.md` file:

```
.blitzkrieg/skills/himalaya/
  SKILL.md
```
...
```bash
blitzkrieg skills list --verbose
```
...
Blitzkrieg has two complementary skill systems:
...
```bash
blitzkrieg skills list              # List all installed skills
blitzkrieg skills list --verbose    # Show requirements and commands
blitzkrieg skills search <query>    # Search skill registry
blitzkrieg skills install <slug>    # Install from registry
blitzkrieg skills update [slug]     # Update skill(s)
blitzkrieg skills uninstall <slug>  # Remove a skill
