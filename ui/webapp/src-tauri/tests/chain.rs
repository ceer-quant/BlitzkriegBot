...
/// and Drop runs on unwind. Losing it would leak a live (dry) engine loop.
...
/// Per-call socket suffix: `cargo test` runs the tests in parallel threads of
/// one process, so the pid alone would collide.
...
fn spawn_core() -> Option<(KillOnDrop, String)> {
    ...
    let sock = std::env::temp_dir()
        .join(format!("bk-chain-{}-{seq}.sock", std::process::id()))
        .to_string_lossy()
        .into_owned();
...
fn desktop_snapshot_reaches_live_core() {
    let Some((core_guard, sock)) = spawn_core() else {
    ...
    let v: Value =
        serde_json::from_str(&out).expect("desktop_snapshot emits valid JSON");
...
fn desktop_command_status_answers() {
    ...
    let v: Value =
        serde_json::from_str(&out).expect("desktop_command emits valid JSON");
