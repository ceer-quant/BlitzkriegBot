...
export async function createNostrChannel(
        ...
        case 'pairing':
          if (!isUserAllowed(event.pubkey)) {
            ...
            if (/^[A-Z0-9]{8}$/.test(potentialCode) && pairing) {
              ...
              if (request) {
                await sendDM(event.pubkey, 'Successfully paired! You can now chat with Blitzkrieg.');
            ...
            if (pairing) {
              ...
              if (code) {
                await sendDM(
                  event.pubkey,
                  `Pairing Required\n\nYour pairing code: ${code}\n\nRun 'blitzkrieg pairing approve nostr ${code}' to complete.\n\nCode expires in 1 hour.`
                );
  ...
  function connectToRelay(url: string): void {
    ...
    ws.on('open', () => {
      ...
      const subId = 'blitzkrieg-' + generateShortId(8);
      ws.send(
        JSON.stringify([
          'REQ',
          subId,
