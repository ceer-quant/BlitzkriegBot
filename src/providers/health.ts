...
const DEFAULT_CONFIG: Required<ProviderHealthConfig> = {
  intervalMs: (() => { const v = Number(process.env.BLITZKRIEG_PROVIDER_HEALTH_INTERVAL_MS); return Number.isNaN(v) ? 30_000 : v; })(),
  failureWarnThreshold: (() => { const v = Number(process.env.BLITZKRIEG_PROVIDER_HEALTH_WARN_AFTER); return Number.isNaN(v) ? 3 : v; })(),
};
