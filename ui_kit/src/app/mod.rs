...
use crate::core::types::{UiSnapshot, CoreEvent};
...
impl AppView {
    pub fn from_snapshot(s: &UiSnapshot) -> Self {
        ...
        v.headline = format!(
            "{} · {} · round #{} {}s left",
            v.mode.to_uppercase(),
            if v.connected { "connected" } else { "DISCONNECTED" },
            v.round_slot,
            v.time_left_sec
        );
...
impl AppViewModel {
    pub fn new(client: IpcClient, trade_limit: usize) -> Self {
        Self { client, trade_limit, view: AppView::default() }
    }
    ...
    pub fn describe_event(ev: &CoreEvent) -> String {
        match ev {
            CoreEvent::OrderUpdate { order } => format!("order {} {} {}", order.order_id, order.status, order.asset),
            CoreEvent::Fill { order, .. } => format!("fill {} {}", order.asset, order.direction),
            CoreEvent::PositionClosed { asset, reason, net_pnl_usd, .. } => {
                format!("closed {asset} {reason} net {net_pnl_usd:+.2}")
            }
            CoreEvent::RiskAlert { message, .. } => format!("RISK: {message}"),
            CoreEvent::ReconcileReport { filled, marked_cancelled, .. } => {
...
pub fn render_headless(v: &AppView) -> String {
    ...
    o.push_str(&format!("│ {}\n", v.headline));
    o.push_str(&format!("│ balance ${:.2} · open {} · trades {} · WR {:.0}% · net {:+.2}\n",
        v.balance, v.open_positions, v.trades, v.win_rate, v.net_pnl));
    if v.rows.is_empty() {
        o.push_str("│ (no open positions)\n");
    } else {
        for r in &v.rows {
            o.push_str(&format!("│ {} {} {:.2}→{:.2} {:+.1}%\n",
                r.asset, r.direction.to_uppercase(), r.entry, r.current, r.unrealized_pct));
        }
    }
