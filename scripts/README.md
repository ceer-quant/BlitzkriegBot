...
| `strategy-no-stop-loss-check.mjs` | DEV_V0_3 §16.4 (E24 / #330): strategy sources (`user_layer/{strategies,parity_strategy,examples,strategies_lua}`) contain zero stop-loss logic and zero reserved intent keys — the static third layer of the「止损不归你管」seal (the kernel's warn-and-drop in `foreign.rs` is the second). `--self-test` pins the scanner on fixtures; `--teeth` injects `local stop = price * 0.965` into an exits reason and must go red with `stop-loss logic found` |
| `strategy-declaration-check.mjs` | DEV_V0_3 §16.4 (E24 / #330): the §7.4 declaration contract end-to-end against a REAL core — 3 legal `bk_strategy_declare_modes` payloads register, 5 illegal ones are refused with the validator's diagnostic (`index=…`, `got="…"`, `Empty`); the macro-declared reference example `momentum_alpha` loads. Drives `user_layer/examples/e24_modes_fixture` (payload from `BK_E24_DECLARATION_FILE` between loads). `--self-test` pins the judge; `--teeth` feeds it a broken-validator output (Empty treated as undeclared / lost `got=`) and must go red; `--compat-dylib P` additionally loads a 0.2-built library unchanged (E24 acceptance #1) |
